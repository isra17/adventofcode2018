...
pub fn solve_part1(squares: &[Square]) -> usize {
    let (_, dup) = squares.iter().fold((HashSet::new(), HashSet::new()), |(mut visited, mut dup), square| {
...
pub fn solve_part2(squares: &[Square]) -> u32 {
    *squares.iter()
        .tuple_combinations()
        .fold(squares.iter().map(|s|s.id).collect::<HashSet<u32>>(),
            |mut candidates, (a,b)| {
...
")), 4);
    ...
    #[test]
    fn examples_part2() {
        assert_eq!(solve_part2(&input_generator("#1 @ 1,3: 4x4
#2 @ 3,1: 4x4
#3 @ 5,5: 2x2
")), 3);
    }
